...
async fn main() {
    ...
    // 1. Initialization
    let config = HuefyConfig::builder().api_key("sdk_lab_test_key").build();
    ...
    if empty_result.is_err() {
        ...
        r.fail(
            "Config validation",
            "expected error for empty API key, got Ok",
        );
    }
    ...
    match sign_payload(b"test_secret", payload.as_bytes()) {
        ...
        Ok(sig) => r.fail(
            "HMAC signing",
            &format!("expected 64-char hex, got {} chars", sig.len()),
        ),
    ...
    if sanitized.contains("user@example.com") {
        r.fail(
            "Error sanitization",
            "email still present after sanitization",
        );
    } else {
    ...
    let has_email = detections
        .iter()
        .any(|d| d.field.as_deref() == Some("email"));
    let has_ssn = detections.iter().any(|d| d.field.as_deref() == Some("ssn"));
    if detections.is_empty() || !has_email || !has_ssn {
        r.fail(
            "PII detection",
            &format!(
                "expected email and ssn detections, got {} total",
                detections.len()
            ),
        );
    } else {
    ...
    if cb.state() == CircuitState::Closed {
        ...
        r.fail(
            ...
            "expected Closed state on new circuit breaker",
        );
    }
